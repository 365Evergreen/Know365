...
const IconCard: React.FC<IconCardProps> = ({ id, name, url, selected, onToggle }) => {
  return (
      ...
      <Stack tokens={{ childrenGap: 8 }} horizontalAlign="center">
        <div style={{ width: 96, height: 96, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {url ? (
            ...
            <img src={url} alt={name || id} style={{ maxWidth: '100%', maxHeight: '100%' }} />
          ) : (
        ...
        <div style={{ textAlign: 'center', minHeight: 36 }}>
          <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name || id}</div>
        </div>
